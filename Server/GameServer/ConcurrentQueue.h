...
public:
	...
	shared_ptr<T> TryPop()
	{
		...
		while (true)
		{
			// 참조권 휙득 (externalCount를 현시점에서 +1 한 애가 이김)
			...
			if (_head.compare.exchange.strong(oldHead, ptr->next))
			{
...
private:
	static void IncreaseExternalCount(atomic<CountedNodePtr>& counter, CountedNodePtr& oldCounter)
	{
		while (true)
		{
			...
			{
				oldCounter.externalCount = newCounter.externalCounter;
				break;
			}
		}
	}
	...
	static void FreeExternalCount(CountedNodePtr& oldNodePtr)
	{
		...
		while (true)
		{
			...
			{
				if (newCounter.internalCount == 0 && newCounter.eexternalCountRemaining == 0)
					delete ptr;
